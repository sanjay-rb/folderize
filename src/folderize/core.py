...
def create_from_yaml(d, base_path="") -> None:
    ...
    resolved_base = os.path.realpath(base_path if base_path else os.getcwd())
    for name, content in d.items():
        if os.path.isabs(name) or ".." in Path(name).parts:
            raise FolderCreationError(name, "Absolute paths and path traversal (..) are not allowed")
        ...
        if resolved_path != resolved_base and not resolved_path.startswith(resolved_base + os.sep):
            raise FolderCreationError(name, "Path escapes the output directory")
        try:
            ...
            raise FolderCreationError(path, str(e)) from e
    ...
    try:
        ...
        if not isinstance(structure, dict):
            raise YamlParseError(
                args.file,
                f"Expected a YAML mapping at root, got {type(structure).__name__}",
            )
